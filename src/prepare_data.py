...
df[['Year', 'Month', 'Date']] = df["InvoiceDate"].apply(lambda x: pd.Series(str(x).split("-")))
...
df_stockdesc.loc[df_stockdesc.StockCode.duplicated(keep=False), "Description"] = "MULTIPLE"
...
df_custcountry.loc[df_custcountry.CustomerID.duplicated(keep=False), "Country"] = "MULTIPLE"
# drop duplicates again
df_custcountry = df_custcountry.drop_duplicates()
...
df2 = df2.reset_index()
df2 = df2.rename(columns={"Quantity": "AvgQuantity", "UnitPrice": "AvgUnitPrice", "Revenue": "AvgRevenue"})
...
df4 = df4.reset_index()
df4 = df4.rename(columns={"Quantity": "AvgQuantity", "UnitPrice": "AvgUnitPrice", "Revenue": "AvgRevenue"})
